...
class MoviesService {
    ...
    func fetchPopularMovies() async throws -> [Movie] {
        guard let url = URL(string: "\(APIConstants.hostUrl)movie/popular?api_key=\(APIConstants.apiKey)") else { return [] }
    ...
    func fetchMovieDetails(movieId: Int) async throws -> MovieDetails? {
        guard let detailsURL = URL(string: "\(APIConstants.hostUrl)movie/\(movieId)?api_key=\(APIConstants.apiKey)&append_to_response=videos") else { return nil }
    ...
    func searchMovies(query: String) async throws -> [Movie] {
        ...
        guard let url = URL(string: "\(APIConstants.hostUrl)search/movie?api_key=\(APIConstants.apiKey)&query=\(encodedQuery)")
        else { return [] }
        let (data, response) = try await URLSession.shared.data(from: url)
        guard let http = response as? HTTPURLResponse else {
            print("SEARCHED MOVIES: FAILED WITH http error")
            throw URLError(.badServerResponse)
        }

        guard (200..<300).contains(http.statusCode) else {
            print("SEARCHED MOVIES: FAILED WITH http error code: \(http.statusCode)")
            throw NSError(domain: "HTTPError", code: http.statusCode)
        }
